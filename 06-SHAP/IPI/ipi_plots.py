...
DATA_CSV = ROOT / "commuting-data" / "data_swindon_with_commute.csv"
...
def fig_investment(curves, top, path):
    # Inserted at \textwidth on an A4 page, so this ~13.8 in canvas is scaled to
    # ~0.46x. The old 8.5-11.5 pt fonts landed at ~4 pt on paper. These enlarged
    # sizes bring the on-page text to ~7-8 pt WITHOUT changing figsize (the
    # panel grid keeps its original aspect ratio); extra row spacing below stops
    # the bigger top-titles/verdicts from colliding between the two rows.
    F_TITLE, F_TOP, F_LABEL, F_TICK, F_VERDICT, F_LEGEND = 18, 16, 16, 15, 15, 16
    n = len(curves)
    ncol = min(n, 3)
    ...
    verdict_texts = []  # (ax, text) pairs, repositioned under each x-label below
    for ax, (_, r) in zip(axes, top.iterrows()):
        ...
        ax.plot(grid, gva, color=NAVY, lw=2.2, marker="o", ms=4, mfc=NAVY,
                mec="white", mew=0.4, zorder=3)
        ax.plot(cur, base, "o", ms=11, mfc=GOLD, mec=NBLACK, mew=1.0, zorder=5)

        if xc is not None:
            ax.plot(xc, target, marker="v", ms=12, mfc=RED, mec="white", mew=0.7, zorder=6)
        ...
        # One grouped 2-line header (id on top, £ context below) so the two lines
        # space themselves instead of colliding as separate annotations.
        ax.set_title(f"#{int(r['rank'])}  {code}\nnow £{base:.0f}m → target £{target:.0f}m",
                     loc="left", fontsize=F_TOP, fontweight="bold", color=NBLACK,
                     linespacing=1.4, pad=8)
        ax.set_xlabel(LABELS.get(feat, feat) + (" (share)" if is_pct(feat) else " (index)"),
                      fontsize=F_LABEL)
        ax.set_ylabel("pred. GVA (£m)", fontsize=F_LABEL)
        ax.tick_params(labelsize=F_TICK)
        # Placed at a provisional y; the real y is measured from the x-label below.
        vtxt = ax.text(0.5, -0.42, verdict, transform=ax.transAxes, ha="center",
                       va="top", fontsize=F_VERDICT, color=vcol)
        verdict_texts.append((ax, vtxt))

    for ax in axes[n:]:
        ax.set_axis_off()

    key = [
        Line2D([0], [0], color=NAVY, lw=2.4, marker="o", ms=5, mfc=NAVY, mec="white", label="model response"),
        Line2D([0], [0], marker="o", color="none", mfc=GOLD, mec=NBLACK, ms=11, label="current level"),
        Patch(fc=GREEN_BAND, ec=GREEN_LINE, label="current → +30% zone"),
        Line2D([0], [0], marker="v", color="none", mfc=RED, mec="white", ms=11, label="lever reaches +30%"),
    ]
    fig.legend(handles=key, loc="lower center", ncol=4, fontsize=F_LEGEND, frameon=False,
               bbox_to_anchor=(0.5, 0.02))
    # Manual spacing (not tight_layout). The generous hspace/bottom leave clear
    # air between the two rows and above the bottom legend.
    fig.subplots_adjust(left=0.075, right=0.985, top=0.905, bottom=0.275,
                        hspace=1.05, wspace=0.28)

    # Drop each verdict a fixed gap below wherever its x-axis label actually
    # ends, measured after layout — so the two never overlap at any font size.
    fig.canvas.draw()
    GAP = 0.14  # axes-fraction gap under the x-label
    for ax, vtxt in verdict_texts:
        ext = ax.xaxis.get_label().get_window_extent()
        y_bottom = ax.transAxes.inverted().transform((ext.x0, ext.y0))[1]
        vtxt.set_position((0.5, y_bottom - GAP))

    # Save the FULL fixed canvas (not bbox="tight") so the panel grid keeps its
    # exact original aspect ratio (figwidth:figheight) regardless of text size.
    fig.savefig(path, bbox_inches=Bbox([[0, 0], [fig.get_figwidth(), fig.get_figheight()]]))
    plt.close(fig)
