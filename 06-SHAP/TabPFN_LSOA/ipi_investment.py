...
def main():
    ipi = pd.read_csv(C.HERE / "ipi_tabpfn_lsoa.csv")
    ...
    for _, r in top.iterrows():
        ...
        lo, hi = float(df[feat].quantile(0.05)), float(df[feat].quantile(0.95))
        grid = np.linspace(lo, hi, GRID)
        rowsX = np.repeat(X[i:i + 1], GRID, axis=0)
        rowsX[:, j] = grid
        gva = np.exp(reg.predict(rowsX))
        cur = float(X[i, j])
        actual = float(np.exp(y[i]))
        base = float(np.interp(cur, grid, gva))
        target = actual * UPLIFT
        xc, direction = C.lever_for_target(grid, gva, cur, target)
        ...
        rows.append({
            ...
            "reachable_within_support": xc is not None,
        })
    ...
    print(inv[["rank", "LSOA21CD", "bottleneck_label", "gva_now_model_m",
               "gva_target_+30pct_m", "lever_now", "lever_at_+30pct", "lever_change",
               "lever_direction", "reachable_within_support"]].to_string(index=False), flush=True)
