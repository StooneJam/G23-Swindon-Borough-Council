...
def perturb_tier(ws: dict, tier_value: float, pct: float) -> dict:
    """Scale every feature currently at `tier_value` by (1+pct), then renormalise the
    whole weight vector so total weight mass over levered (w>0) features is unchanged
    from baseline -- the raw-weight analogue of "renormalise so weights sum to 1"."""
