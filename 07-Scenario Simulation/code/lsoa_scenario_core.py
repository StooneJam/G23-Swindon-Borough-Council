...
ROOT = HERE.parent
FEATURE_FILE = ROOT / "commuting-data" / "data_swindon_with_lsoa_commute.csv"
EXTERNAL_FILE = ROOT / "commuting-data" / "swindon_external_lsoa_commute_summary.csv"
...
def _knn_row_standardised_W(distance, k=8):
    n = distance.shape[0]
    W = np.zeros((n, n))
    for i in range(n):
...
def morans_i(values, W, permutations=999, seed=0):
    ...
    ge = np.sum(perm >= I) + 1
    le = np.sum(perm <= I) + 1
    pseudo_p = min((min(ge, le) / (permutations + 1)) * 2, 1.0)
    return {
        "morans_I": float(I),
...
def gwr_local_elasticity(features, sw_geo):
    ...
    ols_residual = y - X @ global_beta
    W = _knn_row_standardised_W(distance, k=8)
    diagnostics = {
        ...
        "spatial_autocorrelation": {
            "weights": "8-nearest-neighbour, row-standardised",
            "ols_residuals": morans_i(ols_residual, W),
            "gwr_residuals": morans_i(residual, W),
        },
    }
